...
- **[wrapper]** Simplify the desktop Cloud client to `cloakbrowser client`. It now defaults to the deployed control plane, collects credentials on the loopback page, and keeps a revocable device session for 30 days by default without storing the account password. Restarting the client validates and restores that session; explicit sign-out rotates the server credential and deletes the owner-only local session file. Loopback origin checks treat `localhost` and `127.0.0.1` as equivalent only on the same port, avoiding false login rejections while continuing to block external origins.
- **[wrapper]** Add dedicated local and SSH server Docker deployment scripts for the Cloud control plane. The smaller non-root Cloud image keeps encrypted state in a named volume, generates persistent secrets outside the repository, publishes only to host loopback, reports container health, and preserves configuration across repeat deployments. Public server access remains behind an operator-managed HTTPS reverse proxy.
- **[wrapper]** **Assigned cloud workspaces, portable profile keys, and platform administration.** Cloud environments can now be assigned to Member accounts, with API-level scoping across environment lists, Agents, leases, tasks, snapshots, runtime proxy secrets, and extension downloads. Assigned environments must use cloud storage. The new `cloakbrowser workspace` command opens a loopback desktop UI, enrolls a user-bound device with a rotating digest-only credential, and reuses the encrypted snapshot Agent runtime to list, launch, stop, restore, and upload only that user's environments. Members can also create their own `backup` or `shared` environments from the desktop; the server forces organization and self-assignment, encrypts the proxy, and sends the first closed profile through normal snapshot upload. Sensitive runtime credentials remain in the local Python service, with `--cli` retained as a diagnostic fallback. Owners and Admins can manage assignments from the cloud environment editor; desktop credentials cannot be rotated into organization-wide Agent tokens. Cloud-backed macOS and Linux profiles explicitly use portable Chromium key backends and carry encrypted compatibility metadata; incompatible platforms and machine-bound Windows DPAPI snapshots fail before replacing local data. A deployment-controlled superadmin email allowlist grants effective Owner access across all organizations without creating memberships, and records organization entry plus mutations with an explicit platform audit marker. Superadmins now have a platform user directory for provisioning accounts without personal organizations, inspecting team/device counts, activation control, password resets, and cross-team role management without switching organizations. Member-role exit and team removal fail closed around active leases, then revoke that team's desktop credentials and clear environment assignments; last-owner protection and target-team auditing remain enforced.
- **[wrapper]** **Local manager proxy editing, detection, and locale fixes.** A successful proxy check now reports the matched timezone, locale, and WebRTC exit IP and switches the editor to automatic GeoIP mode, clearing stale manual regional overrides. Changing only the protocol selector now rewrites the saved proxy scheme without requiring credentials to be entered again, and unsaved scheme changes are honored by connectivity checks. The proxy field is now plain text and reloads the complete URL through a dedicated local, no-store editor endpoint; list and summary APIs remain masked. Persistent profiles now update both Chromium language preference fields, and the old macOS 145 binary uses a scoped locale fallback so `navigator`, Intl, and `Accept-Language` no longer retain the host's language.
- **[wrapper]** **Cloud team console foundation, remote execution, encrypted profile transfer, and managed runtime assets.** New optional `cloakbrowser[cloud]` extra and `cloakbrowser cloud` command provide a separate FastAPI/SQLAlchemy control plane with registration/login, opaque Argon2id-backed sessions, CSRF-protected mutations, organizations, team switching, Owner/Admin/Operator/Viewer RBAC, normalized environment groups, tenant-isolated environment configuration, revision-based concurrent-edit protection, audit logs, and remote task history. Execution nodes use one-time-displayed, digest-only Agent credentials and run remotely queued persistent Chromium environments through the new `cloakbrowser agent` worker. Task claims have separate expiring credentials; launch completion requires a matching live lease; organization-scoped environment leases are exclusive, require their own secret, and carry monotonically increasing fencing tokens. `backup` and `shared` environments encrypt the complete closed Chromium user-data directory with a per-environment AES-256-GCM key, upload opaque versioned snapshots, and restore them across Agents. Wrapped keys, strict upload validation, quotas, fencing/version checks, safe archive extraction, and persistent dirty-state conflict handling protect the transfer path. Cookies, LocalStorage, IndexedDB, cache, extension state, and history remain isolated under `local` policy or follow the environment under cloud policies. Owner/Admin users can also upload validated immutable extension ZIPs and assign them to environments; Agents cache and safely unpack packages by SHA-256. Proxy credentials are encrypted with the cloud master key, masked in management responses, released only to the current environment lease, kept in Agent memory for launch, and redacted from task errors. SQLite is the local-development default and PostgreSQL is supported for deployment.
- **[wrapper]** **Local browser environment manager.** New `cloakbrowser manager` command opens an AdsPower-style management interface on `127.0.0.1:8765` for creating and reusing persistent browser identities. Python only.
  - **Isolated reusable environments** — every environment has its own persistent Chromium data directory and profile file, keeping cookies, logins, LocalStorage, IndexedDB, cache, service workers, preferences, fingerprint seed, proxy, and regional settings separate. Environments launch in independent persistent browser contexts and can be started, stopped, edited, cloned with a new seed, or moved to a recoverable local trash directory.
  - **Environment organization and bulk operation** — groups, tags, notes, search, status/group filtering, visible-row selection, and batch start/stop controls support larger profile sets. Polling now updates the table only when profile data changes, so an open row action menu or edit workflow is not dismissed by background refreshes.
  - **Proxy management** — HTTP, HTTPS, SOCKS5, and SOCKS5H proxy URLs are accepted, including authenticated proxies. The editor can test an unsaved or saved proxy, display its observed public exit IP, detect changes, lock an environment to the accepted exit IP, block launch on a mismatch, and explicitly accept a new IP. Credentials are masked in API responses and redacted from launch/check errors.
  - **Region and language controls** — primary US IANA timezones, US city geolocation presets, and locale choices including `en-US` and `en-GB` can be selected manually with or without a proxy. Locale preferences are also written into the persistent Chromium profile. GeoIP is disabled automatically when no proxy is configured; with a proxy it can align timezone, locale, and WebRTC IP to the proxy exit.
  - **Seed preview and runtime fingerprint details** — create/edit forms launch an isolated temporary browser to show the actual identity generated by the selected seed before saving. Running environments expose captured User-Agent, platform, languages, timezone, CPU threads, device memory, screen/available area, color depth, viewport/window geometry, device pixel ratio, WebGL vendor/renderer, storage quota, touch points, cookies, and Do Not Track. Secure-context-only device memory is collected through a locally fulfilled temporary loopback page without navigating the user's startup page or making a network request.
  - **Advanced fingerprint overrides** — optional per-environment controls cover platform persona, browser brand/version, platform version, hardware concurrency, device memory, screen size, GPU vendor/renderer, taskbar height, deterministic fingerprint noise, and Chromium 148+ third-party-cookie compatibility. Seed-generated values remain the default; manual values are validated, incomplete GPU pairs and obvious host/platform conflicts are surfaced, and the exact same flags are used by both preview and real launches.
  - **Import, export, and backward compatibility** — selected environments can be exported and imported as JSON. Safe export removes proxy credentials by default, while an explicit credential export preserves them; cookies, browser data, runtime state, and local paths are never included. Older profiles that predate advanced fingerprint fields receive compatible defaults during listing, cloning, export, import, and launch.
  - **Local security boundary** — the server refuses non-loopback binds, protects mutations with a per-process CSRF token and JSON content-type checks, limits request sizes, blocks asset path traversal, stores manager/profile directories with owner-only permissions where supported, writes profile files atomically with restricted permissions, and never exposes raw proxy credentials through the public profile API.
